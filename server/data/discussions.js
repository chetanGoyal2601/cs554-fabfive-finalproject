...
//create new post
async function createPost(userId, eventId, postText) {
  try {
    idValidation(userId);
    ...
    const newPost = {
      ...
      eventId: eventId, //ObjectId(eventId),
      text: postText.trim(),
      comments: [],
      likes: [],
    };
...
async function getPost(postId) {
  try {
    ...
    post.eventId = post.eventId.toString();
...
//get all posts
async function getAllPostsForEvent(eventId) {
  try {
    //idValidation(eventId);
    const postsCollection = await posts();
...
async function createCommentOnPost(userId, postId, text) {
  try {
    ...
    let updatedComment = commentData.get(commentId);
